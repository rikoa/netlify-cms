...
class Editor extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
     ...
     */
    const newSlug =
      nextProps.entryDraft && nextProps.entryDraft.getIn(['entry', 'slug']);
    ...
    if (entry && !entry.get('isFetching') && !entry.get('error')) {
      ...
      const fieldsMetaData =
        nextProps.entryDraft && nextProps.entryDraft.get('fieldsMetaData');
  ...
  handleChangeStatus = newStatusName => {
    ...
    this.props.updateUnpublishedEntryStatus(
      collection.get('name'),
      slug,
      currentStatus,
      newStatus,
