...
const getColumnClassName = columnName => {
  switch (columnName) {
    case 'draft':
      return 'nc-workflow-listDraft';
    case 'pending_review':
      return 'nc-workflow-listReview';
    case 'pending_publish':
      return 'nc-workflow-listReady';
  }
};
...
Please drag the card to the "Ready" column to enable publishing.`,
    ...
    return (
      <div>
        {entries.map(entry => {
          ...
          const author = entry.getIn(
            ['data', 'author'],
            entry.getIn(['metaData', 'user']),
          );
  ...
  render() {
    ...
    return <div className="nc-workflow-list-container">{columns}</div>;
  }
}
