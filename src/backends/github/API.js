...
export default class API {
  ...
  isPrivateRepo() {
    return this.request(this.repoURL).then(repo => repo.private);
  }
  ...
  editorialWorkflowGit(fileTree, entry, filesList, options) {
    ...
    if (!unpublished) {
      ...
      return this.getBranch()
        ...
        .then(branchResponse =>
          this.createPR(options.commitMessage, branchName),
        )
        .then(pr => {
          prResponse = pr;
          return this.user();
  ...
  uploadBlob(item) {
    const content =
      item instanceof AssetProxy ? item.toBase64() : this.toBase64(item.raw);
